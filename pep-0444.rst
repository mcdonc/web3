...
.. note::
   ...
   If the PEP is accepted, and the Web-SIG community agrees, we may
   yet retitle the spec as "WSGI2", with attendant changes to environ
   key names.  In the meantime, if the PEP is not acceped by the
   Web-SIG community, there will be no confusion if a succeeding PEP
   *is* named WSGI2.
...
- Added ``web3.path_requoted`` flag, indicating that ``PATH_INFO`` and
  ``SCRIPT_NAME`` have been reconstructed from an unreliable source.
...
``SCRIPT_NAME``
  ...
  *not* be unquoted: URL-quoted values such as ``%2F`` *will* remain
  present in ``SCRIPT_NAME`` provided to a Web3 application.  If a
  server must reconstruct ``SCRIPT_NAME`` to provide a quoted value,
  it should do so as best it can and set the ``web3.path_requoted``
...
``PATH_INFO``
  ...
  **may** be a bytes instance if the request URL targets the
  ...
  present in ``PATH_INFO`` provided to a Web3 application.  If a
  server must reconstruct ``PATH_INFO`` to provide a quoted value, it
  should do so as best it can and set the ``web3.path_requoted``
  environment value to ``True``.
...
``web3.path_requoted`` The spec requires PATH_INFO and SCRIPT_NAME to
...
``body``).  The ``status`` value is assumed by a gateway or server to
be an HTTP "status" bytes instance like ``b'200 OK'`` or ``b'404 Not
Found'``.  That is, it is a string consisting of a Status-Code and a
...
#) ``__getitem__`` should not be used against str values in Web3 code
...
status and header values returned by a Web3 application.
